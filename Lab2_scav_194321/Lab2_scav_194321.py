...
def cut_video(input):
    ...
    # file = BBB.mp4
    # n = seconds             # idk
    str = "ffmpeg -i " + input + " -ss 00:00:50 -t 00:00:10 -c copy BBB_cuted.mp4"
    runBash(str)
    # Verification
...
# Convert to YUV
def yuv(input):
    str = "ffmpeg -i " + input + " -vf split=2[a][b],[b]histogram, format=yuv420p[hh],[a][hh]overlay videoYUV.mp4"
    runBash(str)
    # file = open(input,'r')  # Open the file
    # str = "ffmpeg -i " + input + " -c:v rawvideo -pix_fmt yuv420p BBBYUV.yuv"
    #str = "ffmpeg -i " + input + " BBBYUV.yuv"
    #runBash(str)
    # YUV to mp4
...
def resize(input):
    # file = open(input,'r')  # Open the file
    #720p:
    str = "ffmpeg -i " + input + " -s hd720 -c:v libx264 -crf 23 -c:a aac -strict -2 BBB720p.mp4"
    runBash(str)
    #480p:
    str2 = "ffmpeg -i " + input + " -s hd480 -c:v libx264 -crf 23 -c:a aac -strict -2 BBB480p.mp4"
    runBash(str2)
    #360x240:
